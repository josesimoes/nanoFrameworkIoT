...
namespace Iot.Device.Hcsr04
{
    ...
    public class Hcsr04 : IDisposable
    {
        ...
        private bool TryGetDistance(out double result)
        {
            ...
            long hangTicks = Environment.TickCount / TimeSpan.TicksPerMillisecond + 100;
            _timer.Reset();
            ...
            {
                Thread.Sleep(TimeSpan.FromMilliseconds(Environment.TickCount / TimeSpan.TicksPerMillisecond - _lastMeasurment));
            }

            // Trigger input for 10uS to start ranging
            _controller.Write(_trigger, PinValue.High);
            Thread.Sleep(TimeSpan.FromMilliseconds((long)(TimeSpan.TicksPerMillisecond * 0.01)));
            _controller.Write(_trigger, PinValue.Low);
            ...
            {
                if (Environment.TickCount / TimeSpan.TicksPerMillisecond - hangTicks > 0)
                {
                    result = default;
                    return false;
                }
            ...
            _lastMeasurment = (int)(Environment.TickCount / TimeSpan.TicksPerMillisecond);
